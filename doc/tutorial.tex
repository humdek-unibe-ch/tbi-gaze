...
\begin{description}
    ...
    \item[Gaze.exe] This program uses the \href{http://developer.tobii.com/tobii-pro-sdk/}{Tobii Pro SDK} to extract the gaze position on the screen where the subject is looking at.
        ...
        \end{itemize}
        Instead the program \texttt{GazeClose.exe} shoudl be used.
    \item[GazeClose.exe] This program requests Gaze.exe to close gracefully and logs these events to the log file.
    \item[GazeRecordingDisable.exe] This program requests Gaze.exe to stop recording gaze data.
    ...
    \item[GazeRecordingEnable.exe] This program requests Gaze.exe to start recording gaze data.
    \item[MouseTrackingDisable.exe] This program requests Gaze.exe to stop updating the mouse pointer by the gaze position.
    \item[MouseTrackingEnable.exe] This program requests Gaze.exe to start updating the mouse pointer by the gaze position.
...
For example, to only store a minimalistic set of data points use the value \texttt{"\{0\}\textbackslash t\{1\}\textbackslash t\{4\}"}.
This will only store the timestamp of when the gaze point was captured by the eye tracker (data field number 0) and the x and y coordinates of the gaze point (data field numbers 1 and 4, respectively).
